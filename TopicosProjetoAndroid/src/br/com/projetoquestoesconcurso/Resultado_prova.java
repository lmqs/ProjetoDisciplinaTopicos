...
public class Resultado_prova extends Activity implements OnItemClickListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		Log.i("Resultado_Prova", "Chegou tem a lista de questões? tamanho: "
				+ questions.size());
	...
	public void calculaMedia() {
		TextView mediaTxt = (TextView) findViewById(R.id.lblmedia);
		int totalQuestoes = listaDeQuestao.size();
		double media = 0;
		for (int i = 0; i < listaDeQuestao.size(); i++) {
			if (listaDeQuestao.get(i).getAlternativaMarcadaUsuario() == listaDeQuestao
					.get(i).setaAlternativaCorreta()) {
				media++;
			}
		}
		mediaTxt.setText("" + media / totalQuestoes);
	}
